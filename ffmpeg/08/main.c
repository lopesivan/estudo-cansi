...
for (;;) {
    ...
    src_data[0] = (uint8_t *)mmap(NULL, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, buf.m.offset);
    if (!src_data[0]) {
        fprintf(stderr, "Could not mmap buffer: %s\n", strerror(errno));
        break;
    }
    src_data[1] = src_data[0] + (buf.length * 2) / 4;
    src_data[2] = src_data[1] + (buf.length * 1) / 4;

    // Convert to YUV420P
    struct SwsContext *sws_ctx = sws_getContext(codecpar->width, codecpar->height, AV_PIX_FMT_YUYV, codecpar->width, codecpar->height, AV_PIX_FMT_YUV420P, 0, NULL, NULL, NULL);
    if (!sws_ctx) {
        fprintf(stderr, "Could not create conversion context\n");
        munmap(src_data[0], buf.length);
        break;
    }
    av_image_fill_arrays(frame->data, frame->linesize, src_data, codecpar->format, codecpar->width, codecpar->height, 1);
    sws_scale(sws_ctx, frame->data, frame->linesize, 0, codecpar->height, frame->data, frame->linesize);
    ...
    if (ioctl(fd, VIDIOC_QBUF, &buf) == -1) {
        fprintf(stderr, "Could not queue buffer: %s\n", strerror(errno));
        break;
    }
}
