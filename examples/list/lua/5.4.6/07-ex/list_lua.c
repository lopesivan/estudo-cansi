...
// Função fechada usada como closure: retorna próximo elemento da iteração
static int l_list_iter_next(lua_State *L)
{
    ListElmt **udata = (ListElmt **)lua_touserdata(L, lua_upvalueindex(1));
    if (*udata == NULL)
    {
        return 0; // fim da iteração
    }

    const char *str = (const char *)list_data(*udata);
    lua_pushstring(L, str ? str : "(null)");

    *udata = list_next(*udata); // avança ponteiro
    return 1;                   // retorna um valor
}

// Função list.iter(lista)
static int l_list_iter(lua_State *L)
{
    List *list = (List *)luaL_checkudata(L, 1, "ListMeta");

    // Aloca espaço para armazenar o ponteiro do elemento atual (userdata
    // leve)
    ListElmt **udata = (ListElmt **)lua_newuserdata(L, sizeof(ListElmt *));
    *udata = list_head(list);

    // Retorna função de iteração (com closure sobre o ponteiro)
    lua_pushcclosure(L, l_list_iter_next, 1);
    return 1;
}
