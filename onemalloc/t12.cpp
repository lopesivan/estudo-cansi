// g++ -std=c++20 -O2 -Wall -Wextra -pedantic t12.cpp -o app
#include <cstddef>
#include <cstdlib>
...
// ---------------------------------------------------------------------------
// TODO: sum_array
// Ranges (C++20/23, mais expressivo)
#include <numeric>
#include <ranges>

template <typename T> T sum_array(T *array, std::size_t size) {
  auto view = std::views::counted(array, size);
  return std::accumulate(view.begin(), view.end(), T{0});
}

// Ou com fold (C++23):
template <std::ranges::range R> auto sum_range(R &&range) {
...
int main() {
  ...
  // Don't touch this!
  // clang-format off
  auto total = sum_array(std::span{*matriz2d});
  std::cout << "sum_array ="
            << total
            << '\n';
  // clang-format on
