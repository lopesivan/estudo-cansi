// g++ -std=c++17 -O2 -Wall -Wextra -pedantic matriz_raii.cpp -o app
#include <cstddef>
#include <cstdlib>
...
int main() {
  constexpr std::size_t dims3D[MAX_DIM] = {2, 3, 2};
  ...
  auto matriz = criar_matriz<Tipo>(3, const_cast<std::size_t *>(dims3D));
  if (!matriz) {
    std::cerr << "Erro ao alocar matriz2d\n";
    return EXIT_FAILURE;
  }

  auto ***matriz3d = static_cast<Tipo ***>(matriz.get());

  Tipo value = 1.0;
  for (std::size_t i = 0; i < dims3D[0]; ++i)
    for (std::size_t j = 0; j < dims3D[1]; ++j)
      for (std::size_t k = 0; k < dims3D[2]; ++k)
        matriz3d[i][j][k] = static_cast<Tipo>(value++);
  ...
  std::cout << "matriz2d[1][1][1] = " << matriz3d[1][1][1] << '\n';
